...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'ptp_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		if ( ptp_acf_options() ) {
			 ...
			 */
			$acf_blog_desc = get_field( 'ptp_meta_blog_description', 'option' );
			...
			$wp_blog_desc = get_option( 'ptp_meta_blog_description' );
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'pt-plugin' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'ptp_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'ptp_meta_description', $description );
