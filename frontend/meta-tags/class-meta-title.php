...
class Meta_Title {
	...
	public function __construct() {
		...
		add_action( 'ptp_meta_title_tag', [ $this, 'title' ] );
	...
	public function title() {
		...
		$author_title = sprintf(
			'%1s %2s',
			__( 'Posts by', 'pt-plugin' ),
			get_the_author_meta( 'display_name', $author_id )
		);

		// Apply a filter to author archive title.
		$author_meta = apply_filters( 'ptp_author_meta_title', $author_title );

		// Custom search title.
		$search_title = sprintf(
			'%1s %2s',
			__( 'Searching', 'pt-plugin' ),
			get_bloginfo( 'name' )
		);

		// Apply a filter to search title.
		$search_meta = apply_filters( 'ptp_search_meta_title', $search_title );
