...
class Frontend {
	...
	public function __construct() {
		...
		// Get inline options.
		$jquery  = get_option( 'ptp_inline_jquery' );
	...
	public function dependencies() {
		...
		// Get inline options.
		$scripts = get_option( 'ptp_inline_scripts' );
		$styles  = get_option( 'ptp_inline_styles' );

		// Add styles inline if option selected.
		if ( $styles ) {
			require_once PTP_PATH . 'frontend/class-styles-inline.php';

		// Otherwise enqueue styles.
		} else {
			require_once PTP_PATH . 'frontend/class-styles-enqueue.php';
		}

		// Add scripts inline if option selected.
		if ( $scripts ) {
			require_once PTP_PATH . 'frontend/class-scripts-inline.php';

		// Otherwise enqueue scripts.
		} else {
			require_once PTP_PATH . 'frontend/class-scripts-enqueue.php';
		}

		// Clean up some scripts in the `head` section.
		require_once PTP_PATH . 'frontend/class-head-scripts.php';

		// Meta tags for SEO.
		include_once PTP_PATH . 'frontend/meta-tags/class-meta-tags.php';
	...
	public function get_jquery() {
		...
		if ( ! is_customize_preview() ) {
			...
			$jquery = file_get_contents( PTP_PATH . '/assets/js/jquery.min.js' );
	...
	public function attachment_fancybox() {
		...
		if ( is_attachment() && get_option( 'ptp_enqueue_fancybox_script' ) ) { ?>
