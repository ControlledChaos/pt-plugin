 ...
 * @link        https://github.com/ControlledChaos/pt-plugin
 * @link        https://controlledchaos.github.io/pt-plugin/
 * @license     GPL-3.0+ http://www.gnu.org/licenses/gpl-3.0.txt
 *
 * Plugin Name:  PT Plugin
 * Plugin URI:   https://github.com/ControlledChaos/pt-plugin
 * Description:  Functionality for the Proud Truther website.
 * Version:      1.0.0
...
if ( ! class_exists( 'PT_Plugin' ) ) :
	 ...
	 *              Change to `self` if used.
		...
		private function constants() {
			 ...
			 */
			if ( ! defined( 'PTP_PATH' ) ) {
				define( 'PTP_PATH', plugin_dir_path( __FILE__ ) );
			}
			 ...
			 * Change the second part of the define(), here as 'pt-plugin',
			 * to your preferred page slug.
			 *
			 * @since  1.0.0
			 * @return string Returns the URL slug of the admin pages.
			 */
			if ( ! defined( 'PTP_ADMIN_SLUG' ) ) {
				define( 'PTP_ADMIN_SLUG', 'pt-plugin' );
			}
			 ...
			 */
			if ( ! defined( 'PTP_DEFAULT_META_IMAGE' ) ) {
				define(
					'PTP_DEFAULT_META_IMAGE',
		...
		public function __clone() {
			...
			_doing_it_wrong( __FUNCTION__, __( 'This is not allowed.', 'pt-plugin' ), '1.0.0' );
		...
		private function dependencies() {
			...
			require_once PTP_PATH . 'includes/class-init.php';

			// Include the activation class.
			require_once PTP_PATH . 'includes/class-activate.php';

			// Include the deactivation class.
			require_once PTP_PATH . 'includes/class-deactivate.php';
	 ...
	 * @since  1.0.0
	 * @access public
	 * @return object Returns the instance of the `PT_Plugin` class.
	 */
	function ptp_plugin() {

		return PT_Plugin::instance();
 ...
 * @return void
 */
register_activation_hook( __FILE__, '\ptp_activate_plugin' );
register_deactivation_hook( __FILE__, '\ptp_deactivate_plugin' );
 ...
 * @return void
 */
function ptp_deactivate_plugin() {
...
function ptp_about_link( $links ) {
	...
	if ( is_admin() ) {
		...
		if ( ptp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'ptp_site_plugin_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'ptp_site_plugin_link_position' );
		}

		if ( true == $position ) {
			$url = admin_url( 'index.php?page=' . PTP_ADMIN_SLUG . '-settings' );
		} else {
			$url = admin_url( 'admin.php?page=' . PTP_ADMIN_SLUG . '-settings' );
		}

		// Create new settings link array as a variable.
		$about_page = [
			sprintf(
				'<a href="%1s" class="' . PTP_ADMIN_SLUG . '-page-link">%2s</a>',
				admin_url( 'plugins.php?page=' . PTP_ADMIN_SLUG . '-page' ),
				esc_attr( 'Documentation', 'pt-plugin' )
			),
		];
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'ptp_about_link' );
 ...
 * @link   https://codex.wordpress.org/Plugin_API/Filter_Reference/plugin_action_links_(plugin_file_name)
 */
function ptp_settings_links( $links, $file ) {
	...
	if ( is_admin() ) {
		...
		if ( ptp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'ptp_settings_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'ptp_site_settings_position' );
		}
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			...
			$links[] = sprintf(
				'<a href="%1s" class="' . PTP_ADMIN_SLUG . '-settings-link">%2s</a>',
				$url,
				esc_attr( 'Site Settings', 'pt-plugin' )
			);
			$links[] = sprintf(
				'<a href="%1s" class="' . PTP_ADMIN_SLUG . '-scripts-link">%2s</a>',
				admin_url( 'options-general.php?page=' . PTP_ADMIN_SLUG . '-scripts' ),
				esc_attr( 'Script Options', 'pt-plugin' )
			);

			// Add a placeholder for an upgrade link.
			$links[] = sprintf(
				'<a href="%1s" title="%2s" class="' . PTP_ADMIN_SLUG . '-upgrade-link" style="color: #888; cursor: default;">%3s</a>',
				''/* Add upgrade URL here */,
				__( 'Upgrade not available', 'pt-plugin' ),
				esc_attr( 'Upgrade', 'pt-plugin' )
			);
...
add_filter( 'plugin_row_meta', 'ptp_settings_links', 10, 2 );
