...
class Admin {
	...
	private function dependencies() {
		...
		require_once PTP_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( ptp_acf_pro() && ! get_option( 'ptp_acf_activate_settings_page' ) ) {
			include_once PTP_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( ptp_acf_pro() ) {
			include_once PTP_PATH . 'admin/class-settings-fields-site-acf.php';
		}
		...
		if ( ( ptp_classicpress() || ptp_new_cms() ) && $editor ) {
			include_once PTP_PATH . 'admin/classic-editor/classic-editor.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once PTP_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once PTP_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once PTP_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once PTP_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( ptp_acf_options() ) {
			include_once PTP_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once PTP_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( ptp_classicpress() ) {
			$default = __( 'website powered by ClassicPress', 'pt-plugin' );

		// If the site is running WordPress.
		} else {
			$default = __( 'website powered by WordPress', 'pt-plugin' );
		}
		...
		if ( ptp_acf_options() ) {
			 ...
			 */
			$credit = get_field( 'ptp_admin_footer_credit', 'option' );
			$link   = get_field( 'ptp_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'pt-plugin' ),
					esc_url( $link ),
					$credit
				);
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'ptp_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'ptp_footer_link' ) );
		...
		$admin_footer = apply_filters( 'ptp_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( PTP_ADMIN_SLUG . '-admin', PTP_URL . 'admin/assets/css/admin.min.css', [], PTP_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( PTP_ADMIN_SLUG . '-tooltips', PTP_URL . 'admin/assets/css/tooltips.min.css', [], PTP_VERSION, 'all' );
		 ...
		 */
		if ( ptp_acf() ) {
			wp_enqueue_style( PTP_ADMIN_SLUG . '-acf', PTP_URL . 'admin/assets/css/acf.css', [], PTP_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'ptp_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( PTP_ADMIN_SLUG . '-welcome', PTP_URL . 'admin/assets/css/welcome.css', [], PTP_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require PTP_PATH . 'admin/partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( PTP_ADMIN_SLUG . '-conditionalize', PTP_URL . 'admin/assets/js/admin.js', [ 'jquery' ], PTP_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( PTP_ADMIN_SLUG . '-admin', PTP_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], PTP_VERSION, true );
