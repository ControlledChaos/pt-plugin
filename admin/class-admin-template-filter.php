...
class Filter_By_Template {
	...
	public function filter_dropdown() {
		...
		<select name="page_template_filter" id="page_template_filter">
			<option value="all"><?php _e( 'All Page Templates', 'pt-plugin' ); ?></option>
			<option value="default" <?php echo ( $template == 'default' ) ? ' selected="selected" ' : ''; ?>><?php echo _e( 'Default Template', 'pt-plugin' ); ?></option>
	...
	public function template_columns_head( $columns ) {
		...
		$columns['template'] = __( 'Template', 'pt-plugin' );
	        ...
	        if ( $template ) {
				...
				// If it's not the default template.
					...
					// If the template cannot be found.
	        			...
	        			echo sprintf(
							'<span title="%1s">%2s</span>',
							__( 'This template file does not exist', 'pt-plugin' ),
							$template
						);
					}
 ...
 */
function ptp_filter_by_template() {
