...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( ptp_acf_options() ) {
            ...
            $admin_header = get_field( 'ptp_use_admin_header', 'option' );
        } else {

            // Otherwise look for the WP API setting.
            $admin_header = get_option( 'ptp_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( ptp_acf_options() ) {
            ...
            // Get the field.
            $link_label = get_field( 'ptp_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'ptp_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'pt-plugin' );
        }
        ...
        if ( ptp_acf_options() ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'ptp_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'ptp_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'pt-plugin' );
        }
    ...
    public function about_plugin_output() {
        ...
        require PTP_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'pt-plugin' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'pt-plugin' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once PTP_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once PTP_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'pt-plugin' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'pt-plugin' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'pt-plugin' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'pt-plugin' )
         );
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'pt-plugin' )
		] );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once PTP_PATH . 'admin/partials/admin-header.php';
		}
	...
	public function admin_header_layout() {
        ...
        $style .= '.gutenberg-editor-page .ptp-admin-header { padding-left: 20px; }';
        $style .= '.gutenberg-editor-page .ptp-admin-header:before { content: ""; display: block; width: 100%; height: 56px; }';

        // End the style block.
        $style .= '</style>';

        // Apply a filter for custom admin themeing.
        $style = apply_filters( 'ptp_admin_header_layout', $style );
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'pt-plugin' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'pt-plugin' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'pt-plugin' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'ptp_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'ptp_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'pt-plugin' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'ptp_image_column_head', $name );
