...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'pt-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'ptp_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'ptp-scripts-general';
    $fields  = 'ptp-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'ptp-scripts-vendor';
    $fields  = 'ptp-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'ptp_section_script_options', $section );
$do_fields  = apply_filters( 'ptp_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'pt-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'pt-plugin' );
} else {
    $save = __( 'Save Settings', 'pt-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'ptp_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'pt-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'pt-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'pt-plugin' ); ?></p>
    <?php endif; ?>
