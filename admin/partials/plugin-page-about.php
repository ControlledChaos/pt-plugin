...
$settings_icon = sanitize_text_field( get_option( 'ptp_site_settings_link_icon' ) );
...
$tabs = [
    ...
    sprintf(
        ...
        '#intro',
        esc_html__( 'Introduction', 'pt-plugin' )
	),
    ...
    sprintf(
		...
		'#settings',
		esc_attr( $settings ),
        esc_html__( 'Site Settings', 'pt-plugin' )
	),
    ...
    sprintf(
        ...
        '#scripts',
        esc_html__( 'Script Options', 'pt-plugin' )
	),
    ...
    sprintf(
        ...
        '#media',
        esc_html__( 'Media Options', 'pt-plugin' )
	),
    ...
    sprintf(
        '<li><a href="%1s"><span class="dashicons dashicons-welcome-learn-more"></span> %2s</a></li>',
        '#tools',
        esc_html__( 'Development Tools', 'pt-plugin' )
    ),

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'ptp_tabs_page_about', $tabs );

?>
<!-- Default WordPress/ClassicPress page wrapper -->
<div class="wrap site-plugin-wrap">
	<!-- Page heading -->
	<?php echo sprintf( '<h1 class="wp-heading-inline">%1s %2s</h1>', get_bloginfo( 'name' ), esc_html__( 'Plugin', 'pt-plugin' ) ); ?>
	<!-- Page description -->
    <p class="description"><?php esc_html_e( 'A feature-packed WordPress or ClassicPress starter plugin for building custom-tailored websites.', 'pt-plugin' ); ?></p>
	...
	<div class="backend-tabbed-content">
		...
		do_action( 'ptp_content_page_about_before' ); ?>
		<!-- Begin content -->
		<div id="intro"><!-- Introduction content -->
			<?php include_once PTP_PATH . 'admin/partials/plugin-page-intro.php'; ?>
		</div>
		<div id="settings"><!-- Site Settings content -->
			<?php include_once PTP_PATH . 'admin/partials/plugin-page-site-settings.php'; ?>
		</div>
		<div id="scripts"><!-- Script Options content -->
			<?php include_once PTP_PATH . 'admin/partials//plugin-page-script-options.php'; ?>
		</div>
		<div id="media"><!-- Media Options content -->
			<?php include_once PTP_PATH . 'admin/partials/plugin-page-media-options.php'; ?>
		</div>
		<div id="tools"><!-- Dev Tools content -->
			<?php include_once PTP_PATH . 'admin/partials/plugin-page-dev-tools.php'; ?>
		</div>
		<?php // Hook for adding tabbed content.
		do_action( 'ptp_content_page_about_after' ); ?>
