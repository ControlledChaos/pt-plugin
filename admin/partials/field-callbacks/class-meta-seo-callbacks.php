...
class Meta_SEO_Callbacks {
	...
	public function disable_meta( $args ) {
		...
		$option = get_option( 'ptp_meta_disable' );

		$html = '<p><input type="checkbox" id="ptp_meta_disable" name="ptp_meta_disable" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="ptp_meta_disable"> '  . $args[0] . '</label></p>';
	...
	public function  schema_org_type( $args ) {
		...
		// Get the field file.
		include_once PTP_PATH . 'admin/partials/field-callbacks/schema-org-type.php';
 ...
 */
function ptp_meta_seo_callbacks() {
