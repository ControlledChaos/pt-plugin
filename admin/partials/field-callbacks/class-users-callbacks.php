...
class Users_Callbacks {
	...
	public function block_gravatar( $args ) {
		...
		$option = get_option( 'ptp_block_gravatar' );

		$html = '<p><input type="checkbox" id="ptp_block_gravatar" name="ptp_block_gravatar" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="ptp_block_gravatar"> ' . $args[0] . '</label></p>';
