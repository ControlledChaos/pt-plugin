...
final class ACF_Tab_Icons {
	...
	public function enqueue_scripts_styles() {
		...
		if ( file_exists( get_theme_file_path( 'assets/fonts/icons/afc-icons.css' ) ) ) {
			wp_register_style( 'acf-tab-icons', get_theme_file_uri( 'assets/fonts/icons/afc-icons.css' ), [], PTP_VERSION );

		// Register tab icons CSS from plugin folder.
		} else {
			wp_register_style( 'acf-tab-icons', PTP_URL . 'assets/fonts/icons/afc-icons.css', [], PTP_VERSION );
		}
	...
	public function icon_register_field_settings( $field ) {
		...
		if ( file_exists( get_theme_file_path() . '/acf-title-icons/selection.json' ) ) {
			...
			$json_file = file_get_contents( PTP_URL . 'assets/fonts/icons/selection.json' );
		}
		...
		if ( ! isset( $json_content['icons'] ) ) {
			...
			acf_render_field_setting( $field, [
				'label'			=> __( 'Icon', 'pt-plugin' ),
				'instructions'	=> '',
				'type'			=> 'message',
				'message'		=> __( 'No icons found', 'pt-plugin' ),
				'new_lines'		=> ''
			] );
		...
		acf_render_field_setting( $field, [
			'label'			=> __( 'Icon', 'pt-plugin' ),
			'instructions'	=> __( 'Select an icon you want to show before the tab text.', 'pt-plugin' ),
			'type'			=> 'select',
		...
		acf_render_field_setting( $field, [
			'label'			=> __( 'Show icon only', 'pt-plugin'),
			'instructions'	=> __( 'If set to <em>Yes</em>, you will see only the icon and no text.', 'pt-plugin'),
			'name'			=> 'show_icon_only',
			'type'			=> 'true_false',
			'ui'			=> 1,
		] );
