...
class Welcome {
    ...
    public function __construct() {
		...
		if ( ptp_acf_options() ) {
			$dismiss = get_field( 'ptp_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
		} else {
			$dismiss = get_option( 'ptp_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( ptp_acf_options() ) {
			$welcome = get_field( 'ptp_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'pt-plugin' ),
			'id'            => 'ptp_welcome_widget_first',
			'description'   => __( '', 'pt-plugin' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'pt-plugin' ),
			'id'            => 'ptp_welcome_widget_second',
			'description'   => __( '', 'pt-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'pt-plugin' ),
			'id'            => 'ptp_welcome_widget_last',
			'description'   => __( '', 'pt-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function welcome_panel() {
		...
		if ( ! empty( $welcome ) ) {
			...
			include_once PTP_PATH . 'admin/dashboard/partials/welcome-panel.php';
		}
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( PTP_ADMIN_SLUG . '-welcome', PTP_URL .  'assets/css/welcome.min.css', [], null, 'screen' );
        }
