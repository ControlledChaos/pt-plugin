...
class Settings_Page_Site {
    ...
    public function settings_page() {
		...
		if ( ptp_acf_options() ) {
			...
			$position   = get_field( 'ptp_settings_link_position', 'option' );

			// The label of the page in the admin menu.
			$link_label = get_field( 'ptp_site_settings_link_label', 'option' );

			// The icon used for the pagin in the admin menu.
			$link_icon  = get_field( 'ptp_site_settings_link_icon', 'option' );
			...
			// Otherwise use "Site Settings" as the label.
				...
				$label = __( 'Site Settings', 'pt-plugin' );
			}
			...
			if ( 'top' == $position ) {
				...
				$settings = apply_filters( 'controlled_chaos_site_settings_page_top', [
					'page_title' => $title . __( ' Settings', 'pt-plugin' ),
					'menu_title' => $label,
					'menu_slug'  => PTP_ADMIN_SLUG . '-settings',
					'icon_url'   => $icon,
					'position'   => 3,
		...
		// If the Advanced Custom Fields Pro plugin is not active.
			...
			// The label of the page in the admin menu.
			$link_label = sanitize_text_field( get_option( 'ptp_site_settings_link_label' ) );

			// The icon used for the pagin in the admin menu.
			$link_icon  = sanitize_text_field( get_option( 'ptp_site_settings_link_icon' ) );
			...
			// Otherwise use the Admin Settings icon.
				...
				$icon = __( 'dashicons-admin-settings', 'pt-plugin' );
			}
			...
			if ( isset( $_GET['page'] ) ) {
				...
				if (
					...
					&& ( $_GET['page'] == PTP_ADMIN_SLUG . '-settings' )
				) {
					wp_redirect( admin_url( 'admin.php?page=' . PTP_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				...
				// If on the top level page and top level option is not selected.
					...
					wp_redirect( admin_url( 'index.php?page=' . PTP_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				}
			}
	...
	public function help_acf_settings_notice() {
		...
		include_once PTP_PATH . 'admin/partials/help/help-acf-notice.php';
    ...
    public function page_help_section_sidebar() {
		...
		$html = '<ul>
			<li><a href="https://www.advancedcustomfields.com/resources/" target="_blank" style="text-decoration: none;">' . __( 'ACF Documentation', 'pt-plugin' ) . '</a></li>
		</ul>';
    ...
    public function page_output() {
		...
		require PTP_PATH . 'admin/partials/settings-page-site.php';
	...
	public function acf_docs_link() {
		...
		if ( ptp_acf_options() ) {
			...
			$submenu['edit.php?post_type=acf-field-group'][] = [ __( 'Documentation', 'pt-plugin' ), 'manage_options', $url ];
 ...
 */
function ptp_settings_page_site() {
