...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'ptp_check_wp_debug_define' );
}

function ptp_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'ptp_site_development' );
		if ( $dev_mode ) {
			ptp_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			ptp_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function ptp_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = ptp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'ptp_set_wp_debug_define', $turn_it_on );
 ...
 */
function ptp_find_wpconfig_path() {
 ...
 * @return string|bool The path of wp-config.php file or false
 */
register_deactivation_hook( __FILE__, 'ptp_deactivation' );

function ptp_deactivation() {
    // set WP_DEBUG back to false
    set_ptp_wp_debug_off( false );
}

function set_ptp_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = ptp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'ptp_set_wp_debug_define', $turn_it_off );
