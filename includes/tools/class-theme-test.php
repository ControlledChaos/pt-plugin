...
// namespace PT_Plugin\Includes\Tools;
...
function themedrive_add_pages() {
    add_theme_page(
        ...
        __( 'Theme Test', 'pt-plugin' ),
        'edit_theme_options',
        PTP_ADMIN_SLUG . '-theme-test',
...
function themedrive_switcher() {
    ...
    if ( themedrive_is_enabled() ) {
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is enabled with', 'pt-plugin' ),
            $theme_name
        );
    } else {
        $get_theme  = wp_get_theme();
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is disabled', 'pt-plugin' ),
            __( 'The active theme is', 'pt-plugin' ),
            $theme_name
        );
    }
...
function themedrive_options_page() {
    ...
    if ( isset( $_POST['button'] ) && 'Enable Theme Drive' == $_POST['button'] ) {
        ...
        $msg_status = __( 'Live theme test has been disabled.', 'pt-plugin' );
...
?>
<div class="wrap" >
    <h2><?php _e( 'Live Theme Test', 'pt-plugin' ); ?></h2>
    <?php $action_url; ?>
    ...
    <?php wp_nonce_field( 'theme-drive' ); ?>
        <h2><?php _e( 'Instructions', 'pt-plugin' ); ?></h2>
        <ol>
            <li><?php _e( 'Select a theme to preview live on the site from the select box below (lists all installed themes).', 'pt-plugin' ); ?></li>
            <li><?php _e( 'Enable theme test', 'pt-plugin' ); ?></li>
            <li><?php _e( 'Once the theme is ready to go live, disable theme test and activate the theme on the Themes page.', 'pt-plugin' ); ?></li>
        </ol>
   	    <p><?php _e( 'Additionally you may add "?theme=xxx" to a URL, where xxx is the slug of the theme you want to test.', 'pt-plugin' ); ?></p>
        <?php themedrive_switcher(); ?>
        <h2><?php _e( 'User Role', 'pt-plugin' ); ?></h2>
        <p><?php _e( 'Specify the level of users to have access to the selected theme preview.', 'pt-plugin' ); ?></p>
        <p><?php _e( 'By default it is set to 10 (admin only). Level 7 are editors, level 4 are authors and level 1 are contributors.', 'pt-plugin' ); ?></p>
        <p><?php _e( 'The access level is ignored for accessing the site with ?theme=xxx paramaeter.', 'pt-plugin' ); ?></p>
        <p><input style="border-width: inherit; border-style: inherit; border-color: inherit; width: 30px;" name="access_level" id="access_level" value="<?php echo esc_attr( $access_level ); ?>" /> <label for="access_level"><?php _e( 'Access level', 'pt-plugin' ); ?></label></p>
        <p class="submit">
