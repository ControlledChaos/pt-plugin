...
if ( ! class_exists( 'DB_Reset_Admin' ) ) :
	...
	class DB_Reset_Admin {
		...
		private function handle_after_reset() {
			...
			$this->notice_success = __( 'The selected tables were reset', 'pt-plugin' );
		...
		private function assert_correct_code() {
			...
			if ( $this->request['db-reset-code'] !== $this->request['db-reset-code-confirm'] ) {
				$this->notice_error = __( 'You entered the wrong security code', 'pt-plugin' );
				return false;
			}
		...
		private function load_javascript_vars() {
			...
			return [
				'confirmAlert' => __( 'Are you sure you want to continue?', 'pt-plugin' ),
				'selectTable'  => __( 'Select Tables', 'pt-plugin' )
			];
