...
// namespace PT_Plugin\Includes\Tools\Database_Reset;
...
<div class="wrap">
	<h2><?php _e( 'Database Reset', 'pt-plugin' ) ?></h2>
	<p class="description"><?php _e( 'Reset the WordPress database back to its original state.', 'pt-plugin' ); ?></p>
	<hr />

	<?php include( 'partials/notice.php' ) ?>

	<form method="post" id="db-reset-form">
		<p>1. <?php _e( 'Select the database tables you would like to reset', 'pt-plugin' ) ?>:</p>

		<div id="select-container">
			<a href='#' id="select-all"><?php _e( 'Select All', 'pt-plugin' ) ?></a>
		...
		<p id="reactivate" style="display: none">
			...
			<?php _e( 'Reactivate current theme and plugins after reset?', 'pt-plugin' ) ?>
			</label>
		</p>

		<p id="disclaimer" style="display: none">
			<em><?php printf( __( 'You selected the users table. Only the <strong><u>%s</u></strong> user will be restored', 'pt-plugin' ), $this->user->user_login ) ?>.</em>
		</p>
		<p>
		2. <?php _e( 'Enter the security code into the text box', 'pt-plugin' ) ?>:
		...
		<p>
			<input type="submit" name="db-reset-submit" value="<?php _e( 'Reset Tables', 'pt-plugin' ) ?>" id="db-reset-submit" class="button-primary" disabled />
