...
namespace PT_Plugin\Includes\Media;
...
class Media {
	...
	private function dependencies() {
		...
		include_once PTP_PATH . 'includes/media/class-svg-support.php';

		// Replace WP gallery shortcode if Fancybox option is used.
		$fancybox = get_option( 'ptp_enqueue_fancybox_script' );

		if ( $fancybox ) {
			require_once PTP_PATH . 'includes/media/class-gallery-shortcode.php';
		}
	...
	public function image_sizes() {
		...
		add_image_size( __( 'Meta Image', 'pt-plugin' ), 1200, 630, true );
		 ...
		 */
		add_image_size( __( 'Column Thumbnail', 'pt-plugin' ), 48, 48, true );
	...
	public function rss_featured_images( $content ) {
		...
		$size = apply_filters( 'ptp_rss_featured_image_size', 'medium' );
