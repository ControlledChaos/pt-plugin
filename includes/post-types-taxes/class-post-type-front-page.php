...
class Post_Types_Front_Page {
	...
	public function pre_get_posts( $query ) {
		...
		if ( $query->is_home() && $query->is_front_page() && $query->is_main_query() ) {
			...
			$post_type  = get_option( 'ptp_front_page_post_type', '' );
	...
	public function customizer( $wp_customize ) {
		...
		$wp_customize->add_setting( 'ptp_front_page_post_type', [
			'type'       => 'option',
			'capability' => 'manage_options',
			'default'    => 'post',
		]);

		$wp_customize->add_control( 'ptp_front_page_post_type', [
			'label'           => __( 'Front Page Post Type', 'pt-plugin' ),
			'type'            => 'radio',
			'choices'         => $post_types,
