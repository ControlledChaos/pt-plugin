 ...
 *
 * @since      1.0.0
 * @author     Greg Sweet <greg@ptpzine.com>
...
class Posts_Per_Page {
	...
	public function activate() {
		...
		$current_options   = get_option( 'ptp_ppp_options' );
		$default_options   = [];
		...
		foreach ( $option_type_array as $option_type ) {
			...
			} elseif ( ! empty( $ptp_ppp_options[ $option_type . '_count' ] ) ) {
		...
		update_option( 'ptp_ppp_options', $default_options );
	...
	public function add_settings() {
		add_options_page( __( 'Posts Per Page', 'pt-plugin' ), __( 'Posts Per Page', 'pt-plugin' ), 'manage_options', 'posts-per-page', [ $this, 'view_settings' ] );
	}
	...
	public function view_settings() {
		...
		<div class="wrap posts-per-page">
			...
			<h1><?php _e( 'Posts Per Page', 'pt-plugin' ); ?></h1>
			<h2><?php _e( 'Overview', 'pt-plugin' ); ?></h2>
			...
			can be set for any other pages not covered by this.', 'pt-plugin' ); ?></p>
			<p><?php _e( 'The initial value used on activation was pulled from the setting <em>Blog Pages show at most</em> found in the', 'pt-plugin' ); ?> <a href="<?php echo site_url( '/wp-admin/options-reading.php' ); ?>" title="Reading Settings"><?php _e( 'Reading Settings', 'pt-plugin' ); ?></a></p>
			<form method="post" action="options.php">
				<?php
					settings_fields( 'ptp_ppp_options' );
					do_settings_sections( 'ptp_ppp' );
					do_settings_sections( 'ptp_ppp_custom' );
				?>
			<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'pt-plugin' ); ?>" /></p></form>
		</div>
		<?php
	}
	...
	public function register_settings() {
		...
		register_setting( 'ptp_ppp_options', 'ptp_ppp_options', [ $this, 'validate_options'] );

		add_settings_section( 'ptp_ppp_section_main', '', [ $this, 'output_main_section_text' ], 'ptp_ppp' );
		add_settings_section( 'ptp_ppp_section_custom', '', [ $this, 'output_custom_section_text' ], 'ptp_ppp_custom' );

		add_settings_field( 'ptp_ppp_index_count',     __( 'Main Index posts per page', 'pt-plugin' ), [ $this, 'output_index_count_text' ],    'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_category_count',  __( 'Category posts per page', 'pt-plugin' ),   [ $this, 'output_category_count_text' ], 'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_archive_count',   __( 'Archive posts per page', 'pt-plugin' ),    [ $this, 'output_archive_count_text' ],  'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_tag_count',       __( 'Tag posts per page', 'pt-plugin' ),        [ $this, 'output_tag_count_text' ],      'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_author_count',    __( 'Author posts per page', 'pt-plugin' ),     [ $this, 'output_author_count_text' ],   'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_search_count',    __( 'Search posts per page', 'pt-plugin' ),     [ $this, 'output_search_count_text' ],   'ptp_ppp', 'ptp_ppp_section_main' );
		add_settings_field( 'ptp_ppp_default_count',   __( 'Default posts per page', 'pt-plugin' ),    [ $this, 'output_default_count_text' ],  'ptp_ppp', 'ptp_ppp_section_main' );

		add_settings_field( 'ptp_ppp_post_type_count', '', [ $this, 'output_post_type_count_text' ], 'ptp_ppp_custom', 'ptp_ppp_section_custom' );
	...
	public function output_main_section_text() {
		?>
		<h2><?php _e( 'Main Settings', 'pt-plugin' ); ?></h2>
		...
		that view and will instead allow default values to pass through.', 'pt-plugin' ); ?></p>
		<p><?php _e( 'Please Note', 'pt-plugin' ); ?>:
		...
		the box on the RIGHT controls the number of posts seen on pages 2, 3, 4, etc... of that view.', 'pt-plugin' ); ?></em></p>
		<?php
	}
	...
	public function output_custom_section_text() {
		...
		?>
		<h2><?php _e( 'Custom Post Type Specific Settings', 'pt-plugin' ); ?></h2>
	...
	public function output_post_type_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options' );
		...
		$args            = apply_filters( 'ptp_output_post_type_count_text', $args );
		...
		foreach ( $all_post_types as $p => $k ) {
			 ...
			 *  assign a good value. */
			if ( empty( $ptp_ppp_options[ $p . '_count' ] ) ) {
				$ptp_ppp_options[ $p . '_count' ] = 0;
			}

			if ( empty( $ptp_ppp_options[ $p . '_count_paged' ] ) ) {
				$ptp_ppp_options[ $p . '_count_paged' ] = 0;
			}
			...
			<tr>
				...
				<td>
					<label for="ptp_ppp_post_type_count[<?php echo esc_attr( $p ); ?>]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_post_type_count[<?php echo esc_attr( $p ); ?>]" name="ptp_ppp_options[<?php echo esc_attr( $p ); ?>_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ $p . '_count' ] ); ?>" />
					&nbsp;
					<label for="ptp_ppp_post_type_count[<?php echo esc_attr( $p ); ?>]"><?php _e( 'Subsequent Pages:', 'tims' ); ?></label> <input id="ptp_ppp_post_type_count[<?php echo esc_attr( $p ); ?>]" name="ptp_ppp_options[<?php echo esc_attr( $p ); ?>_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ $p . '_count_paged' ] ); ?>" />
				</td>
			</tr>
			<?php
		}
	...
	public function output_index_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'front_count' => 0, 'front_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_index_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_index_count[0]" name="ptp_ppp_options[front_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'front_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_index_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_index_count[1]" name="ptp_ppp_options[front_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'front_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_category_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'category_count' => 0, 'category_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_category_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_category_count[0]" name="ptp_ppp_options[category_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'category_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_category_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_category_count[1]" name="ptp_ppp_options[category_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'category_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_archive_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'archive_count' => 0, 'archive_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_archive_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_archive_count[0]" name="ptp_ppp_options[archive_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'archive_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_archive_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_archive_count[1]" name="ptp_ppp_options[archive_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'archive_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_tag_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'tag_count' => 0, 'tag_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_tag_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_tag_count[0]" name="ptp_ppp_options[tag_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'tag_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_tag_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_tag_count[1]" name="ptp_ppp_options[tag_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'tag_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_author_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'author_count' => 0, 'author_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_author_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_author_count[0]" name="ptp_ppp_options[author_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'author_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_author_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_author_count[1]" name="ptp_ppp_options[author_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'author_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_search_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'search_count' => 0, 'search_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_search_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_search_count[0]" name="ptp_ppp_options[search_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'search_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_search_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_search_count[1]" name="ptp_ppp_options[search_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'search_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function output_default_count_text() {
		...
		$ptp_ppp_options = get_option( 'ptp_ppp_options', [ 'default_count' => 0, 'default_count_paged' => 0 ] );

		?>
		<label for="ptp_ppp_default_count[0]"><?php _e( 'First Page:', 'tims' ); ?> </label> <input id="ptp_ppp_default_count[0]" name="ptp_ppp_options[default_count]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'default_count' ] ); ?>" />
		&nbsp;
		<label for="ptp_ppp_default_count[1]"><?php _e( 'Subsequent Pages:', 'tims' ); ?> </label> <input id="ptp_ppp_default_count[1]" name="ptp_ppp_options[default_count_paged]" size="3" type="text" value="<?php echo esc_attr( $ptp_ppp_options[ 'default_count_paged' ] ); ?>" />
		<?php
	}
	...
	public function modify_query( $query ) {
		...
		if ( $query->is_home() ) {
			$this->process_options( 'front', $ptp_ppp_options );
		} elseif ( $query->is_post_type_archive( $post_type_array ) ) {
			$current_post_type_object = $query->get_queried_object();
			$this->process_options( $current_post_type_object->name, $ptp_ppp_options );
		} elseif ( $query->is_category() ) {
			$this->process_options( 'category', $ptp_ppp_options );
		} elseif ( $query->is_tag() ) {
			$this->process_options( 'tag', $ptp_ppp_options );
		} elseif ( $query->is_author() ) {
			$this->process_options( 'author', $ptp_ppp_options );
		} elseif ( $query->is_search() ) {
			$this->process_options( 'search', $ptp_ppp_options );
			...
			$this->process_options( 'archive', $ptp_ppp_options );
		} else {
			$this->process_options( 'default', $ptp_ppp_options );
		}
	 ...
	 * @param  $ptp_ppp_options array of options from the database for custom posts per page.
	 * @return void
	 */
	public function process_options( $option_prefix, $ptp_ppp_options ) {

		if ( ! $this->paged_view && ! empty( $ptp_ppp_options[ $option_prefix . '_count' ] ) ) {

			$this->final_options['posts']  = $ptp_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['offset'] = 0;
			$this->final_options['set_count'] = $ptp_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $ptp_ppp_options[ $option_prefix . '_count_paged' ];

		} elseif ( $this->paged_view & ! empty( $ptp_ppp_options[ $option_prefix . '_count_paged' ] ) ) {

			$this->page_count_offset = ( $ptp_ppp_options[ $option_prefix . '_count_paged' ] - $ptp_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['offset']  = ( ( $this->page_number - 2 ) * $ptp_ppp_options[ $option_prefix . '_count_paged' ] + $ptp_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['posts']   = $ptp_ppp_options[ $option_prefix . '_count_paged' ];
			$this->final_options['set_count'] = $ptp_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $ptp_ppp_options[ $option_prefix . '_count_paged' ];
