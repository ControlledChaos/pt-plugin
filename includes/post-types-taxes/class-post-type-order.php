...
if ( ptp_acf_options() ) {
    $sort_order = get_field( 'ptp_use_custom_sort_order', 'option' );

// If Advanced Custom Fields is not active.
} else {
    $sort_order = get_option( 'ptp_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'ptp_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'ptp_order_install', 1 );
    ...
    public function admin_menu() {
        ...
        add_options_page(
            __( 'Posts & Taxonomies Sort Order', 'pt-plugin' ),
            __( 'Sort Order', 'pt-plugin' ),
            'manage_options',
            'sort-order-settings',
            [ $this, 'admin_page' ]
        );
    ...
    public function admin_page() {
        ...
        require PTP_PATH . 'admin/partials/settings-page-posts-order.php';
    ...
    public function load_script_css() {
        ...
        if ( $this->_check_load_script_css() ) {
            ...
            wp_enqueue_script( 'post-tax-order', PTP_URL . 'admin/assets/js/post-tax-order.js', [ 'jquery' ], null, true );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['ptp_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'ptp_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'ptp_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $ptp_order_options = get_option( 'ptp_order_options' ) ) {
            $ptp_order_options = get_option( 'ptp_order_options' );
        } else {
            $ptp_order_options = [];
        }

        if ( isset( $ptp_order_options['objects'] ) && is_array( $ptp_order_options['objects'] ) ) {
            $objects = $ptp_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $ptp_order_options['tags'] ) && is_array( $ptp_order_options['tags'] ) ) {
            $tags = $ptp_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function ptp_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'ptp_order_install' );
